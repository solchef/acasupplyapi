    ...
    return new Promise((resolve, reject) => {
        try {
            let path = `${__dirname}/coins/${id}.js`,
                ...
                doctrine.parse(metaRaw[0], { unwrap: true, recoverable: true }) : { tags: [] },
                metaObj = {}
                ...
                try {
                    ...
                    res(async(response) => {
                        if (!(response instanceof Error)) {
                            ...
                            cache.set(
                                id,
                                response, !_.isUndefined(meta.ethContractAddr) ?
                                ethTokenCacheTTL :
                                opts.cacheTTL
                            );
                        }
...
yargs.usage('$0 <cmd> [args]')
    ...
    .command('get-all', 'Get supplies for all known coins', (yargs) => {}, function(argv) {
